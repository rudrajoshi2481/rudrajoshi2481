
## 👋 Rudhra Joshi

[![LinkedIn Badge](https://img.shields.io/badge/-LinkedIn-blue?style=flat-square\&logo=Linkedin\&logoColor=white)](https://www.linkedin.com/in/rudrajoshi2481/)
[![Website Badge](https://img.shields.io/badge/-Website-black?style=flat-square\&logo=Safari\&logoColor=white)](https://www.rudhrajoshi.me/)

> **Bioinformatician • Software Developer • Pharmacy Graduate • Deep Learning Engineer**

> 🚀 Building foundation models: **ESM-2, AlphaFold2, DNABERT**

* **MS in Bioinformatics** at Rowan University
* **Pharmacy Graduate** with hands-on experience in drug discovery and medicinal chemistry (anticancer drug)
* **Foundation models**: ESM-2, AlphaFold2, DNABERT
* **Bioinformatics**: RNA-seq, ChIP-seq, Hi-C, ChIA-PET, HiChIP
* **Software Developer**: Next.js, Python, Docker, AWS

### **Tech Stack**

* **Languages**: Python, TypeScript (Next.js)
* **Tools & Frameworks**: Docker, AWS, GCP, Redis, MongoDB, Git, Blender
* **Bioinformatics Tools**: Cooler, Pairtools, BWA-MEM, MACS3, HiGlass, DESeq2, GATK & many more

### 🧩 **Current Focus**

* Learning how to build advanced foundation models in genomics
* Working on genomics pipeline development

### 📫 **Let’s Connect**

* 🌐 Website: [rudhrajoshi.me](https://www.rudhrajoshi.me/)
* 💬 LinkedIn: [rudhrajoshi](https://www.linkedin.com/in/rudhrajoshi/)
* 🌍 Open to new collaborations and ideas
